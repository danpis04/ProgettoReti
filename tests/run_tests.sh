...
check_duplicate_port() {
    ...
    start_utente 5679 /dev/null "$u1_log"
    sleep 1

    if "$ROOT/utente" 5679 < /dev/null > "$u2_log" 2>&1; then
        fail "rifiuto porta P2P duplicata"
    fi

    pass "rifiuto porta P2P duplicata"
    cleanup
}
...
check_full_even_flow() {
    ...
    cleanup
    start_lavagna "$lavagna_log"
    start_utente 5679 /dev/null "$WORKDIR/full_u5679.log"
    start_utente 5680 /dev/null "$WORKDIR/full_u5680.log"
    start_utente 5681 /dev/null "$WORKDIR/full_u5681.log"
    start_utente 5682 /dev/null "$WORKDIR/full_u5682.log"
...
check_terminal_commands() {
    ...
    start_lavagna "$lavagna_log"
    start_utente 5679 "$u1_fifo" "$WORKDIR/commands_u5679.log"
    start_utente 5680 "$u2_fifo" "$WORKDIR/commands_u5680.log"
    sleep 2
